...
######
#set pdf format
pdf(file="/Users/guest1/xin/peaks_methamphetamine.pdf",width = 6.2, height = 8,pointsize = 12, onefile = TRUE)
par(mfrow=c(4,3))

#data.methamphetamine
data.methamphetamine=cbind(data[,97:99],data[,37:48])

outfile=cbind(data.all[,1],data.methamphetamine)
out.file1=gsub(".txt", "methamphetamine.txt", file.data)
write.table(outfile,out.file1,row.names=FALSE,col.names=TRUE)

#rename data.methamphetamine
colnames(data.methamphetamine)<-c("0hr","0hr","0hr","1hr","1hr","1hr","2hr","2hr","2hr","4hr","4hr","4hr","8hr","8hr","8hr")
hours<-rep(NA,ncol(data.methamphetamine))
hours.test<-c("0hr","0hr","0hr","1hr","1hr","1hr","2hr","2hr","2hr","4hr","4hr","4hr","8hr","8hr","8hr")

for (hour.test in hours.test) {
 hours[grep(hour.test, colnames(data.methamphetamine))]<-hour.test
...
for (i.row in 1:nrow(data.methamphetamine)) {
    # for (i.row in 1:10) {
    i.row=1##############
    if (ceiling(i.row/100)==floor(i.row/100)) cat(i.row, "\n")
    data.1=data.methamphetamine[i.row,]
...
# predict(fit.datamy.optnn, c(0,1,2,4,8)) #da1=preplot(fit.datamy.optnn, where="fitp", band="local",get.data=TRUE)
## Find peaks ##
...
source("listIO.R")
writelist(idx.peak.all, filename="idx.peakallmet.txt")
writelist(sig.peak.all, filename="sig.peakallmet.txt")
writelist(de.peak.all, filename="de.peakallmet.txt")
